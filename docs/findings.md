...
This is the full analysis log: raw output tables, explanations, and caveats, in the order the work was actually done. The [top-level README](../README.md) has the short version; this doc is where the evidence lives. Sections are additive — later sections correct or supersede numbers from earlier ones where noted (most importantly, the "Layer 1 final conclusion" section supersedes the plain "Layer 1 conclusion" section, and the "Capped-address correction" section supersedes the raw `activeSpanDays`/cohort-retention numbers from the Layer 1 raw output above it).
...
`pnlInv.py` re-ran on the same underlying tables as the "Churn investigation" section directly above (liquidation counts, worst-trade loss share, escalation ratio, final-week intensity) while chasing the "why does churn happen" question via a lifetime-quintile PnL angle — raw output is identical to the tables already shown above, so it isn't repeated here. What's new is the explanation pass that follows, which is where the "picking up pennies in front of a steamroller" hypothesis actually gets tested and rejected in favor of gradual disengagement.
...
#### Capped-address correction

This supersedes the raw `activeSpanDays` / cohort-retention numbers in the Layer 1 raw output above.
...
**Layer 3 — market microstructure (funding rate).** Planned but not executed: connecting funding-rate extremes (via Hyperliquid's `fundingHistory` info endpoint) to trader-type activity, and checking whether funding is a hidden PnL component not captured in `closedPnl`. Descoped for time — it requires pulling an entirely new dataset (fundingHistory isn't derivable from the existing fills/leaderboard data) with an uncertain payoff, versus the Layer 1/2 work above which already produced concrete, mechanism-level findings. Left here as a documented scope decision rather than an unexplained gap.